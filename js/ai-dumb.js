...
TicTacToe.prototype.aiTurn = function() {
  var choice = false;
  var gameStateString = this.gameState.toString();
  while (!choice) {
    var random = Math.floor(Math.random() * 9);
    if (gameStateString.charAt(9 - random) === "0") {
  ...
  if (this.gameState.toString().charAt(10 - selectedSquare.toString().length) != "0") {
    ...
    if (!this.checkForWin(this.playerTurn)) {
      if (turnCount === 9) {
        ...
        this.playerTurn = 2;
  //-- This line turns on the AI behavior --//
        this.aiTurn();
...
TicTacToe.prototype.checkForWin = function(player) {
  ...
  for (var i = 1; i < gameStateString.length; i += 3) {
    winningScore = (gameStateString.charAt(i) === player.toString()) +
     ...
     (gameStateString.charAt(i + 2) === player.toString())
     if (winningScore === 3) {
  ...
  for (var i = 1; i <= 3; i ++) {
    winningScore = (gameStateString.charAt(i) === player.toString()) +
    (gameStateString.charAt(i + 3) === player.toString()) +
    (gameStateString.charAt(i + 6) === player.toString())
    if (winningScore === 3) {
      alert("Player " + player + " wins!");
      initBoard();
      return true;
